# career_guidance_finetune.py
# ==============================
# Robust multi-model QLoRA finetuning for Career Guidance Counsellor QA
# ==============================
...
MODELS_CONFIG = {
    "tinyllama-career-guidance": {
        ...
        "repo_name": "tinyllama-career-guidance",
    ...
    "mistral-career-guidance": {
        "base_model": "mistralai/Mistral-7B-v0.1",
        "repo_name": "mistral-career-guidance",
        "target_modules": ["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"],
        "lora_rank": 32,
        "max_length": 768,
        "batch_size": 2,
    ...
    "phi2-career-guidance": {
        "base_model": "microsoft/phi-2",
        "repo_name": "phi2-career-guidance",
    ...
    "distilgpt2-career-guidance": {
        ...
        "repo_name": "distilgpt2-career-guidance",
    ...
    "gemma2b-career-guidance": {
        "base_model": "google/gemma-2b",
        "repo_name": "gemma2-career-guidance",
        "target_modules": ["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"],
        "lora_rank": 24,
        "max_length": 640,
        "batch_size": 3,
    ...
    "llama71b-career-guidance": {
        "base_model": "meta-llama/Llama-3.1-70B-Instruct",
        "repo_name": "llama71b-career-guidance",
        "target_modules": ["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"],
        "lora_rank": 64,
        "max_length": 2048,
        "batch_size": 1,
...
# ----------------------------
def validate_conversation(question: str, answer: str) -> Tuple[bool, str]:
    ...
    if len(question) > 1200 or len(answer) > 2000:
        return False, "Too long"
    question_words = question.split()
    answer_words = answer.split()
    if len(question_words) < 3 or len(answer_words) < 8:
        return False, "Not enough words"
    if len(answer_words) > 0:
        from collections import Counter
        most_common = Counter(answer_words).most_common(1)[0][1]
        if most_common / len(answer_words) > 0.25:
            return False, "Too repetitive"
    if answer.strip().lower() in ["ok", "yes", "no", "sure", "okay"]:
        return False, "Too simple response"
    return True, "Valid"

def process_career_guidance_dataset() -> Dataset:
    print("=== Loading Career Guidance Counsellor QA Dataset ===")
    ...
    for ex in dataset:
        ...
        ok, reason = validate_conversation(question, answer)
        validation_stats[reason] += 1
        if ok:
            # Simple format: Question then Answer, no special tokens or system prompts
...
def prepare_dataset():
    processed = process_career_guidance_dataset()
...
**Fine-tuned on:** advy/career-guidance-counsellor-QA — Career guidance Q&A dataset.

## Overview
This model was fine-tuned using LoRA adapters (QLoRA-style 4-bit quantized base model) on the Career Guidance Counsellor QA dataset to improve performance for career guidance conversational responses.
...
## Notes
- Dataset used: advy/career-guidance-counsellor-QA
- This model is intended for career guidance assistance and educational purposes.
"""
...
def train_model(model_config: Dict, train_dataset, val_dataset, test_dataset, model_key: str):
    ...
    try:
        ...
        trainer.push_to_hub(commit_message=f"Finetune on Career Guidance QA - eval_loss: {metrics['evaluation'].get('eval_loss'):.4f}")
...
# ----------------------------
def main():
    print("Career Guidance QA multi-model finetune - upgraded pipeline")
    ...
    os.makedirs("./results_career_guidance", exist_ok=True)
    with open("./results_career_guidance/career_guidance_training_results.json", "w") as f:
        json.dump(results, f, indent=2)

    print("Complete. Results saved to ./results_career_guidance/")
